...
# Functie om data op te halen + succesvol (get) statuscode_gegevens ophalen (hulpfunctie)
def data_ophalen(endpoint):
    response = requests.get(base_url + endpoint)
    if response.status_code == 200:
        return response.json()[0]
    elif response.status_code == 404:
        print("Land of hoofdstad niet gevonden. Controleer de invoer.")
        return None
    else:
        print(f"Fout: API geeft status {response.status_code}")
        return None
